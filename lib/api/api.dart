...
class Api {
  ...
  static const searchMoviesUrl = 
      'https://api.themoviedb.org/3/search/movie?query=Jack+Reacher&api_key=${Constants.apiKey}';
 ...
 Future<List<Movie>> searchMovies(String query) async {
    final response = await http.get(Uri.parse(searchMoviesUrl + query));
    if (response.statusCode == 200) {
      final decodedData = json.decode(response.body)['results'] as List;
      return decodedData.map((i) => Movie.fromJson(i)).toList();
    } else {
